...
% Uses: elf_summary_descriptormean, elf_plot_summary
% Which in turn use elf_plot_int and elf_plot_spatial
...
for setnr = 1:length(frange)
    ...
    thisim = elf_readwrite(para, 'loadHDR_tif', sprintf('scene%03d', frange(setnr)));
    if res.calib
...
res.data = elf_readwrite(para, 'loadres', fnames_im);
res.para = para;
...
set(hi2, 'ButtonDownFcn', @elf_callbacks_explore, 'UserData', res);
