function info = elf_imfinfo(fullfilename)
% ELF_IMFINFO loads EXIF information for an image file and returns it as a struct.
% If nformation for a particular filetype is in a different spot than in a DNG, it should be copied over in this function. 
%
% info = elf_imfinfo(fullfilename)
...
switch lower(ext(2:end))
    case {'tif', 'tiff'}
        info = info(1); %%CHECK WHETHER THIS MAKES SENSE 20150320
        info.ColorMatrix2       = 0;
    case {'jpg', 'jpeg'}
...
%         end
    otherwise
        warning('Unknown format: If fields in the EXIF data are different from TIF, errors will occur. In that case, please create an entry in elf_imfinfo.m for this image format.');
end
