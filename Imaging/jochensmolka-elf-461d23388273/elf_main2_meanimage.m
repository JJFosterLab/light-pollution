...
%
% Uses: elf_support_logmsg, elf_paths, elf_para, elf_readwrite, 
%       elf_plot_image, elf_analysis_int, elf_support_formatA4l,
%       elf_plot_summary_int
%
...
for imnr = 1:length(allfiles)
    thisim     = elf_readwrite(para, 'loadHDR_tif', fnames_im{imnr});  % output is uint16
...
if verbose
    ...
    hi2 = montage(thumbs);
    res.root = fullfile(para.paths.datapath, para.paths.scenefolder);
...
%% Save output to tif
elf_readwrite(para, 'savemeanimg_tif', '', uint16(meanimage));
elf_readwrite(para, 'savemeanimg_jpg', '', uint16(meanimage));
