function meandata = elf_analysis_datasetmean(data, sel, intonly, intmeantype)
...
%   intmeantype       - str, determines which type of averaging to use, should be set to para.plot.datasetmeantype (current default: logmean)
% Outputs:
...
% See also:   elf_main3_intsummary, elf_analysis, elf_plot_summary

%% Check inputs
if nargin < 4 || isempty(intmeantype), intmeantype = 'logmean'; end
...
% mean
meandata.int.means = sub_mean(data(sel), 'int', 'means', intmeantype);

% std
meandata.int.std = sub_mean(data(sel), 'int', 'std', intmeantype);

% min
temp = sub_extract(data, 'int', 'means') - sub_extract(data, 'int', 'min');
meandata.int.minrange = sub_average(temp(:, :, sel), 3, intmeantype);
meandata.int.min = meandata.int.means - meandata.int.minrange;

% max
temp = sub_extract(data, 'int', 'max') - sub_extract(data, 'int', 'means');
meandata.int.maxrange = sub_average(temp(:, :, sel), 3, intmeantype);
meandata.int.max = meandata.int.means + meandata.int.maxrange;

% median
meandata.int.median = sub_mean(data(sel), 'int', 'median', intmeantype);

% 25th percentile
temp = sub_extract(data, 'int', 'median') - sub_extract(data, 'int', 'perc25');
meandata.int.perc25range = sub_average(temp(:, :, sel), 3, intmeantype);
...
meandata.int.perc75range = sub_average(temp(:, :, sel), 3, intmeantype);
...
meandata.int.percminrange = sub_average(temp(:, :, sel), 3, intmeantype);
...
meandata.int.percmaxrange = sub_average(temp(:, :, sel), 3, intmeantype);
meandata.int.percmax = meandata.int.median + meandata.int.percmaxrange;

%% Calculate whole-image means for intensity descriptors
% mean
meandata.totalint.mean = sub_mean(data(sel), 'totalint', 'mean', intmeantype);

% std
meandata.totalint.std = sub_mean(data(sel), 'totalint', 'std', intmeantype);

% min
temp = sub_extract(data, 'totalint', 'mean') - sub_extract(data, 'totalint', 'min');
meandata.totalint.minrange = sub_average(temp(:, :, sel), 3, intmeantype);
...
meandata.totalint.maxrange = sub_average(temp(:, :, sel), 3, intmeantype);
meandata.totalint.max = meandata.totalint.mean + meandata.totalint.maxrange;

% median
meandata.totalint.median = sub_mean(data(sel), 'totalint', 'median', intmeantype);

% 25th percentile
temp = sub_extract(data, 'totalint', 'median') - sub_extract(data, 'totalint', 'perc25');
meandata.totalint.perc25range = sub_average(temp(:, :, sel), 3, intmeantype);
...
meandata.totalint.perc75range = sub_average(temp(:, :, sel), 3, intmeantype);
...
meandata.totalint.percminrange = sub_average(temp(:, :, sel), 3, intmeantype);
...
meandata.totalint.percmaxrange = sub_average(temp(:, :, sel), 3, intmeantype);
meandata.totalint.percmax = meandata.totalint.median + meandata.totalint.percmaxrange;
% 
% % histograms
meandata.int.totalhist = sub_mean(data(sel), 'totalint', 'hist', intmeantype); %% FIXME: This has to take into account BINS!
