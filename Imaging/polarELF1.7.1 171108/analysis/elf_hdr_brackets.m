...
try
    ...
    warning('Exposure bracketing could not be detected due to missing or corrupt exif information. Images will be assumed to be independent (i.e. no bracketing). Original internal error message: %s', me.message);
...
while putative_rep <= 11 && isnan(bracketing_rep)
    if mod(length(ev), putative_rep) == 0 % only check this length if it is a divider of the length of the dataset
        pattern = ev(1:putative_rep);     % the first N elemebts of ev determine the putative pattern
