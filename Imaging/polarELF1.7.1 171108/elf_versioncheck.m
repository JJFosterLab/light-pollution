...
if verLessThan('matlab', '8.0')
    if verbose
        elf_support_logmsg('\n');
        elf_support_logmsg('      This program has only been tested for version 8.0 and above. Please report any errors or bugs.\n');
    end
    warning('This program has only been tested for version 8.0 and above. Please report any errors or bugs.');
end
if ~osfine
    if verbose
        elf_support_logmsg('\n');
...
if ~license('test', 'Image_Toolbox') 
    warning('No Image Processing Toolbox license found. This program will probably not run properly.');
elseif verbose
    elf_support_logmsg('      Image Processing Toolbox license found.\n');
end
if ~license('test', 'Statistics_Toolbox')
    warning('No Statistics Toolbox license found. This program will probably not run properly.');
elseif verbose
    elf_support_logmsg('      Statistics Toolbox license found.\n');
end
if ~verLessThan('matlab', '8.0') && osfine && verbose && license('test', 'Image_Toolbox') && license('test', 'Statistics_Toolbox')
