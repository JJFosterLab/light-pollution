...
%       elf_analysis_int, elf_support_formatA4l, elf_plot_summary_int
%
...
%       15s total
% Typical timing PER SCENE (on ELFPC):
%     3.5s for intensity analysis
%     1.8s to save jpg
%       1s for reading and calibrating
%      <1s rest
...
for setnr = 1:size(sets, 1)
    ...
    for i = 1:numims % for each image in this set
        ...
        im_raw                  = elf_imread(fname);   % load the image (uint16)
    ...
    elf_plot_summary_int(para, res, I, infosum, fh, sprintf('%s, scene #%d of %d', para.paths.dataset, setnr, size(sets, 1)), ...
    ...
    if savejpgs, elf_readwrite(para, 'saveivep_jpg', [scenename '_int'], fh); end    % small bottleneck
