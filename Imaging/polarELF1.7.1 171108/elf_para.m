...
% rootdir = NaN means reset all local folders
...
%% find root folder
mayusegpu = false; % this flag can be used to manually deactivate use of GPU computing
[para.syst, para.thisv, para.usegpu] = elf_versioncheck(verbose, mayusegpu); % Check operating system and Matlab version

if ~isempty(rootdir) && ~any(isnan(rootdir)) && exist(rootdir, 'file')
    ...
    if isnan(rootdir)
        para.paths.root             = elf_io_localpaths('loadroot', 1);
        para.paths.outputfolder     = elf_io_localpaths('loadoutput', 1);
        para.paths.outputfolder_pub = elf_io_localpaths('loadoutput_pub', 1);
    else
        para.paths.root             = elf_io_localpaths('loadroot', 0);
        para.paths.outputfolder     = elf_io_localpaths('loadoutput', 0);
        para.paths.outputfolder_pub = elf_io_localpaths('loadoutput_pub', 0);
    end
end
