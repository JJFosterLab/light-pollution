function night_HDRscenes(dataset, horlimit)
...
for setnr = 1:size(sets, 1)
    ...
    for i = 1:numims % for each image in this set
        ...
        [im_cal(:, :, :, i), conf(:, :, i)] = elf_calibrate_abssens(im_proj, info(imnr) );%, calib); 
%         [im_cal(:, :, :, i), conf(:, :, i)] = veps_calibrate_abssens(im_proj, info(imnr) );%, calib); 
    end
