function data = elf_readwrite(para, action, fname, varinput)
% ELF_READWRITE is the main function for reading and writing data files
...
% present [para = elf_readwrite(para, 'createfilenames')].
%
% Inputs:
...
% para            = elf_readwrite(para, 'loadpara')    %all that para needs for this one is para.paths.fname_infosum_mat

% MAIN1: elf_main1_HDRscenes
%                   elf_readwrite(para, 'saveinfosum', [], infosum)
%                   elf_readwrite(para, 'saveproj_tif', fname, im)
%                   elf_readwrite(para, 'saveHDR_tif', scenename, im_HDR)
%                   elf_readwrite(para, 'saveHDR_mat', scenename, im_HDR)

% MAIN2: elf_main2_meanimage
% infosum         = elf_readwrite(para, 'loadinfosum')
% im_HDR          = elf_readwrite(para, 'loadHDR_tif', scenename)
%                   elf_readwrite(para, 'savemeanimg_tif', '', meanimage)
%                   elf_readwrite(para, 'savemeanimg_jpg', '', meanimage)

% MAIN3: elf_main3_luminance
% infosum         = elf_readwrite(para, 'loadinfosum')
% data            = elf_readwrite(para, 'loadproj_tif', fname, info)
% im_HDR          = elf_readwrite(para, 'loadHDR_tif', scenename)
%                   elf_readwrite(para, 'saveres', fname, data)
%                   elf_readwrite(para, 'saveivep_jpg', fname, fh)

% MAIN3p5: elf_main3p5_intsummary
% infosum         = elf_readwrite(para, 'loadinfosum')
% data            = elf_readwrite(para, 'loadres', fname)
...
% infosum         = elf_readwrite(para, 'loadinfosum')
% data            = elf_readwrite(para, 'loadHDR_mat', scenename)
%                   elf_readwrite(para, 'savefilt_mat', scenename, im_filt_HDR)

% MAIN5: elf_main5_contrasts
% im_filt         = elf_readwrite(para, 'loadfilt_mat', scenename)
% data            = elf_readwrite(para, 'loadres', fname)
% im_HDR          = elf_readwrite(para, 'loadHDR_tif', scenename)
%                   elf_readwrite(para, 'saveres', fname, data)
%                   elf_readwrite(para, 'savevep_jpg', fname, fh)

% MAIN6: elf_main6_summary
% data            = elf_readwrite(para, 'loadres', fname)
%                   elf_readwrite(para, 'savemeanres', '', meandata)
...
%% Main
switch action
    case 'createfilenames' % para = elf_readwrite(para, 'createfilenames')
...
%         end
    ...
    case 'saveinfosum'      % elf_readwrite(para, 'saveinfosum', [], infosum)
    ...
    case 'loadpara'         % infosum = elf_readwrite(para, 'loadpara')
        %% load the para structure from the infosum file for this environment
        temp        = load(para.paths.fname_infosum_mat);
        data        = temp.para;
        
	case 'saveproj_tif'     % elf_readwrite(para, 'saveproj_tif', fname, im)
    ...
    case 'loadproj_tif'     % data = elf_readwrite(para, 'loadproj_tif', fname)
        %% returns a uint16 LINEAR image for one exposure
        [~,f]       = fileparts(fname); 
        fname       = fullfile(para.paths.datapath, para.paths.projfolder, [f '.tif']);
        data        = imread(fname);         % load .tif-file

    case 'saveproj_mat'     % elf_readwrite(para, 'saveproj_mat', fname, im_proj)
    ...
    case 'loadproj_mat'     % im_proj = elf_readwrite(para, 'loadproj_mat', fname)
    ...
    case 'saveHDR_mat'      % elf_readwrite(para, 'saveHDR_mat', sprintf('scene%03d', setnr), im_HDR)
        %% saves the HDR image for one scene in a mat file
        [~,f]       = fileparts(fname);
        fname       = fullfile(para.paths.datapath, para.paths.scenefolder, [f '.mat']);
        save(fname, 'varinput');

    case 'loadHDR_mat'      % data = elf_readwrite(para, 'loadHDR_mat', sprintf('scene%03d', setnr))
    ...
    case 'saveHDR_tif'      % elf_readwrite(para, 'saveHDR_tif', sprintf('scene%03d', setnr), im_HDR)
    ...
    case 'loadHDR_tif'      % im_HDR = elf_readwrite(para, 'loadHDR_tif', sprintf('scene%03d', setnr))
        %% loads the HDR image for one scene from tif
        [~,f]       = fileparts(fname); 
        fname       = fullfile(para.paths.datapath, para.paths.scenefolder, [f '.tif']);
        data        = imread(fname);

    case 'savefilt_mat'     % elf_readwrite(para, 'savefilt_mat', sprintf('scene%03d', setnr), im_filt_HDR)
        ...
        im_filt_HDR = varinput; %#ok<NASGU> % this is only necessary for backward compatibility. TODO: refilter all datasets so they contain varinput instead of im_filt_HDR
        save(fname_filt, 'im_filt_HDR');
                            elf_support_logmsg('      Filtered %s saved to %s\n', f, fname);
                            
    case 'loadfilt_mat'     % im_filt = elf_readwrite(para, 'loadfilt_mat', sprintf('scene%03d', setnr))
...
%     case 'savepolar'     % elf_readwrite(para, 'savepolar', [], pol)
%         fname = fullfile(para.paths.root, sprintf('polresults.mat'));
%         save(fname, 'varinput');
%                             elf_support_logmsg('      Polarisation results saved to %s\n', fname);
%         
%     case 'loadpolar'     % pol = elf_readwrite(para, 'loadpolar')
%         fname = fullfile(para.paths.root, sprintf('polresults.mat'));
%         temp = load(fname);
%         data = temp.varinput;        
        
    case 'savestokes_mat'     % elf_readwrite(para, 'savestokes_mat', sprintf('scene%03d', setnr), imStokes_filt)
        ...
        imStokes_filt = varinput; %#ok<NASGU> % this is only necessary for backward compatibility. TODO: refilter all datasets so they contain varinput instead of im_filt_HDR
        save(fname_filt, 'imStokes_filt');
                            elf_support_logmsg('      Stokes parameters for dataset %s saved to %s\n', f, fname_filt);
                            
    case 'loadstokes_mat'     % imStokes_filt = elf_readwrite(para, 'loadstokes_mat', sprintf('scene%03d', setnr))
    ...
    case 'saveres'          % elf_readwrite(para, 'saveres', fname, data)
    ...
    case 'loadres'          % data = elf_readwrite(para, 'loadres', fname)
    ...
    case 'savemeanimg_tif'  % elf_readwrite(para, 'savemeanimg_tif', '', meanimage)
    ...
    case 'savemeanimg_jpg'  % elf_readwrite(para, 'savemeanimg_jpg', '', meanimage)
    ...
    case 'loadmeanimg_tif'  % meanim = elf_readwrite(para, 'loadmeanimg_tif')
        %% loads mean image for an environment from tif
        data        = imread(para.paths.fname_meanimg_tif);

    case 'savemeanimg_ind'  % elf_readwrite(para, 'savemeanimg_ind', '', meanimage)
    ...
    case 'loadmeanimg_ind'  % meanim = elf_readwrite(para, 'loadmeanimg_ind')
    ...
    case 'savemeanres'      % elf_readwrite(para, 'savemeanres', '', meandata)
    ...
    case 'loadmeanres'      % meandata = elf_readwrite(para, 'loadmeanres')
        %% load mean results mat for an environment
        temp        = load(para.paths.fname_meanres);
        data        = temp.varinput;
          
    case 'savemeanres_int'      % elf_readwrite(para, 'savemeanres_int', '', intmean)
        %% saves mean results mat for an environment; this is called at the end of intensity calculations, and then only saves intensity data
    ...
    case 'loadmeanres_int'      % intmean = elf_readwrite(para, 'loadmeanres_int')
        %% load mean results mat for an environment
        temp        = load(para.paths.fname_meanres_int);
        data        = temp.varinput;
        
	case 'savecollres'      % elf_readwrite(para, 'savecollres', '', data)
    ...
    case 'loadcollres'      % data = elf_readwrite(para, 'loadcollres')
        %% loads collated results mat for an environment
        temp        = load(para.paths.fname_collres);
        data        = temp.varinput;        
        
    case 'savemeanres_comb'      % elf_readwrite(para, 'savemeanres', '', meandata)
        %% saves mean results mat for an environment; this is called at the end of intensity calculations, and then only saves intensity data
    ...
    case 'loadmeanres_comb'      % meandata = elf_readwrite(para, 'loadmeanres')
        %% load mean results mat for an environment
        [~,ds]      = fileparts(para.paths.dataset);
    ...
    case 'savevep_jpg'      % elf_readwrite(para, 'savevep_jpg', fname, fh)
    ...
    case 'savevep_pdf'      % elf_readwrite(para, 'savevep_pdf', fname, fh)
    ...
    case 'saveivep_jpg'     % elf_readwrite(para, 'saveivep_jpg', fname, fh)
    ...
    case 'saveivep_pdf'     % elf_readwrite(para, 'saveivep_pdf', fname, fh)
    ...
    case 'savemeanvep_jpg'  % elf_readwrite(para, 'savemeanvep_jpg', '', fh)
    ...
    case 'savemeanvep_pdf'  % elf_readwrite(para, 'savemeanvep_pdf', '', fh)
    ...
    case 'savemeanivep_jpg' % elf_readwrite(para, 'savemeanivep_jpg', '', fh)
    ...
    case 'savemeanivep_pdf' % elf_readwrite(para, 'savemeanivep_pdf', '', fh)
...
function sub_savepdf(fh, filename)
    elf_plot_hideui(fh, false); % hide user interface for plotting
%     orient(fh, ori);
...
%     set(fh, 'PaperPositionMode', 'Auto', 'PaperUnits', 'centimeters', 'PaperSize', [pos(3), pos(4)]);
    ...
    elf_plot_hideui(fh, true); % re-activate user interface
end
