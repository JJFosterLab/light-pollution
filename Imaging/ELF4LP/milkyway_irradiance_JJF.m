function irrad = milkyway_irradiance_JJF(filename)
...
for sc = 1:4
    ...
    coscorr = cosd(veps_support_sphdist(AZ, EL, 0, 0));
    
    temp = solidangle .* coscorr .* im(:, :, 1);
    irrad(sc, 1) = nansum(temp(:));
    temp = solidangle .* coscorr .* im(:, :, 2);
    irrad(sc, 2) = nansum(temp(:));
    temp = solidangle .* coscorr .* im(:, :, 3);
    irrad(sc, 3) = nansum(temp(:));
